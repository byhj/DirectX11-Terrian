...
namespace byhj
{
	...
	{
		HRESULT hr;
		auto heightMapDir = d3d::TextureMgr::getInstance()->getDir() + "heightmap01.bmp";
		auto colorMapDir = d3d::TextureMgr::getInstance()->getDir() + "colorm01.bmp";
		load_heightMap(heightMapDir.c_str());
	    load_colorMap(colorMapDir.c_str());
		...
		for (int i = 0; i != m_VertexCount; ++i)
		{
			m_VertexData[i].Pos    = TerrainMesh.VertexData[i].Pos;
			m_VertexData[i].Normal = TerrainMesh.VertexData[i].Normal;
			m_VertexData[i].Tex    = TerrainMesh.VertexData[i].Tex;
			m_VertexData[i].Color  = m_ColormapData[i];
		}
	...
	void Terrain::load_colorMap(const char *filename)
	{
		BITMAPFILEHEADER  bitmapFileHeader;
		BITMAPINFOHEADER  bitmapInfoHeader;
		int imageSize;
		...
		for (int j = 0; j != m_TerrainHeight; ++j)
		{
			for (int i = 0; i != m_TerrainWidth; ++i)
			{
				...
				m_ColormapData.push_back(Color);
