...
public:
	...
	void v_Render()
	{
		...
		XMMATRIX tView  = m_camera.GetViewMatrix();
		XMMATRIX tProj  = m_camera.GetProjMatrix();
		m_grid.Render(m_pD3D11DeviceContext, Model, tView, tProj);

		XMMATRIX MVP ;
		XMMATRIX sphereWorld = XMMatrixScaling(10.0f, 10.0f, 10.0f);
		MVP   = XMMatrixTranspose( sphereWorld * View * Proj); 
		m_skymap.Render(m_pD3D11DeviceContext, MVP);
...
void D3DRenderSystem::init_object()
{
	...
	m_skymap.init_shader(m_pD3D11Device, m_hWnd);
